...
class LoginManager:
    ...
    def __init__(self):
        ...
        if (
                frappe.local.form_dict.get(
                    "cmd") == "login" or frappe.local.request.path == "/api/method/login"
        ):
    ...
    def login(self):
        if frappe.get_system_settings("disable_user_pass_login"):
            frappe.throw(
                _("Login with username and password is not allowed."), frappe.AuthenticationError)
    ...
    def set_user_info(self, resume=False):
        ...
        if self.info.user_type == "Website User":
            ...
            if not resume:
                ...
                frappe.local.response["home_page"] = "/app/userinterface"
        ...
        frappe.local.cookie_manager.set_cookie(
            "user_image", self.info.user_image or "")
    ...
    def authenticate(self, user: str = None, pwd: str = None):
        ...
        if not (user and pwd):
            user, pwd = frappe.form_dict.get(
                "usr"), frappe.form_dict.get("pwd")
        if not (user and pwd):
        ...
        ignore_tracker = should_run_2fa(
            user.name) and ("otp" in frappe.form_dict)
    ...
    def force_user_to_reset_password(self):
        ...
        if reset_pwd_after_days:
            last_password_reset_date = (
                frappe.db.get_value("User", self.user,
                                    "last_password_reset_date") or today()
            )
    ...
    def validate_hour(self):
        ...
        login_before = int(frappe.db.get_value(
            "User", self.user, "login_before", ignore=True) or 0)
        login_after = int(frappe.db.get_value(
            "User", self.user, "login_after", ignore=True) or 0)
